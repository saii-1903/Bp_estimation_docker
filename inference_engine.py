...
class VitalInferenceEngine:
        ...
        try:
            ...
            if os.path.exists(cfg.BP_MODEL_CONFIG["classifier"]):
                ...
                for group in ["hypo", "normal", "hyper"]:
                    ...
                    # Use per-group scaler if available, else fall back to global scaler
                    if scl_path and os.path.exists(scl_path):
                        models[f"bp_scaler_{group}"] = joblib.load(scl_path)
                    else:
                        models[f"bp_scaler_{group}"] = models["bp_global_scaler"]
            ...
            print(f"[OK] Loaded models from {cfg.MODEL_DIR}")
    ...
    def _hb_glu_features(self, ppg_seg):
        ...
        peak_amps  = [np.max(cyc) for cyc in cycles]
        median_amp = np.median(peak_amps)
        cycle = min(cycles, key=lambda cyc: abs(np.max(cyc) - median_amp))
    ...
    def predict_vitals(
        ...
        try:
            ...
            SEGMENTS = len(ppg_segment) // SEG_LEN            # 5 for PlethWave[32:], 6 for full 30s

            # Accept 25s+ (PlethWave[32:] from NISO204 = 3568 samples = 29.7s)
            if len(ppg_segment) < FS * 25:
                print(f"DEBUG BP: need 25s ({FS*25} samples), have {len(ppg_segment)} - skipping")
        ...
        try:
            ...
            if hg_raw is not None and "hb_model" in self.models:
                ...
                # [age, age**2, is_senior, gender_bin, age*gender, bmi]
